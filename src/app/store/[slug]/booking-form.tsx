                        ...
                        <div className="space-y-4 pb-2">
                            ...
                            <RadioGroup value={selectedStaff} onValueChange={setSelectedStaff} className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                                <div className="border p-4 rounded-xl hover:border-primary hover:bg-stone-50 cursor-pointer flex flex-col justify-center items-center text-center transition-colors relative" onClick={() => setSelectedStaff('no-preference')}>
                                    ...
                                    <div className="w-16 h-16 mb-2 rounded-full bg-stone-100 flex items-center justify-center text-stone-400">
                                ...
                                {staffList.map((staff) => (
                                    <div key={staff.id} className="border p-4 rounded-xl hover:border-primary hover:bg-stone-50 cursor-pointer flex flex-col transition-colors relative" onClick={() => setSelectedStaff(staff.id)}>
                                        <RadioGroupItem value={staff.id} id={staff.id} className="absolute top-4 right-4" />
                                        <div className="flex items-center gap-3 mb-3">
                                            <div className="w-16 h-16 shrink-0 overflow-hidden rounded-full border-2 border-stone-100">
                                                {staff.avatarUrl ? (
                                                    <img src={staff.avatarUrl} alt={staff.name} className="w-full h-full object-cover" />
                                                ) : (
                                                    <div className="w-full h-full bg-stone-200 flex items-center justify-center text-stone-500 font-bold text-xl">{staff.name.charAt(0)}</div>
                                                )}
                                            </div>
                                            <div>
                                        ...
                                        {staff.tags && staff.tags.length > 0 && (
                                            <div className="flex flex-wrap gap-1 mb-2">
                                                {staff.tags.map(tag => (
                                                    <span key={tag} className="px-1.5 py-0.5 bg-stone-100 text-stone-600 text-[10px] rounded-sm">{tag}</span>
                                                ))}
                                            </div>
                                        )}
                                        {staff.greeting_message && (
                                            <div className="mt-auto pt-2 border-t border-stone-100">
                                                <p className="text-[11px] text-stone-600 line-clamp-2 leading-relaxed">{staff.greeting_message}</p>
                                            </div>
                                        )}
                                    </div>
                                ))}
